...
```{r, message=FALSE}
map<-map%>% addCircles(data = macleish_layers[["challenge_courses"]], 
              weight = 1,popup = "Challenge Course", color = "green" ) #adding the challenge courses to map  
%>%
  addPolylines(data = inside_streams, weight= 5, popup =  ~Id, color = "Gray") #only mapping the streams that are inside the boundary of MacLeish
```

```{r}
map<-map %>%
  addPolylines(data = inside_trails, weight = 5, popup =  ~name, color = ~color1(name)) #mapping the trails and coloring them based on name and adding a popup to show the name of each trail by clicking on it
```
